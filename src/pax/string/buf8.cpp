...
buf8_write_codepoint_head(buf8* self, u32 value)
{
    ...
    utf8_encode_forw_circ(self->memory, self->length,
        self->head, value);
...
{
    uptr size = pax_min(self->length - self->size, value.length);
    uptr prev = self->head + self->length - size;
...
buf8_write_str16_head(buf8* self, str16 value)
{
    ...
    for (uptr i = 0, j = 0; j < size;) {
        ...
        i += utf16_decode_forw(value.memory, value.length, i, &unicode);
...
buf8_write_str32_head(buf8* self, str32 value)
{
    ...
    for (uptr i = 0, j = 0; j < size;) {
        ...
        i += utf32_decode_forw(value.memory, value.length, i, &unicode);
...
{
    uptr size = utf8_decode_forw_circ(self->memory, self->length,
        self->head, value);
...
buf8_read_codepoint_tail(buf8* self, u32* value)
{
    ...
    uptr size = utf8_decode_back_circ(self->memory, self->length,
        prev % self->length, value);
