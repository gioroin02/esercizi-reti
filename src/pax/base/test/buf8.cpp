...
show_buf8(buf8* self)
{
    ...
    for (isiz i = 0; i < self->size; i += 1) {
        isiz idx = (self->head + i) % self->length;
...
main(int argc, char** argv)
{
    ...
    buf8_write_mem8_tail(&buffer, pax_as_u8p("coda"), 4);
    buf8_write_mem8_head(&buffer, pax_as_u8p("testa"), 5);
    ...
    for (isiz i = 0; i < pax_array_bytes(temp); i += 1)
        printf("%llu. %c\n", i, temp[i]);
