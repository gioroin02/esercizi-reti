#ifndef PAX_STRUCTURE_HASH_MAP_CPP
#define PAX_STRUCTURE_HASH_MAP_CPP
...
hash_map_reserve(Arena* arena, isiz length, Hash_Proc<Key>* proc)
{
    ...
    result.outer = table_reserve<usiz>(arena, length);
    result.inner = array_reserve<usiz>(arena, 0);
...
hash_map_clear(Hash_Map<Key, Val>* self)
{
    ...
    for (isiz i = 0; i < self->outer.length; i += 1)
...
hash_map_insert(Hash_Map<Key, Val>* self, Key key, Val value)
{
    ...
    usiz hash  = self->hash(key);
    isiz index = hash % self->outer.length + 1;
    usiz inner = 0;
    usiz outer = 0;
...
hash_map_remove(Hash_Map<Key, Val>* self, Key key, Val* value)
{
    ...
    array_remove(&self->inner, inner - 1, pax_null(usiz));
...
#endif // PAX_STRUCTURE_HASH_MAP_CPP
